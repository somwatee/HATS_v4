...
#include "features.mq5"      // ต้องมีฟังก์ชัน GetRealTimeFeatures()
...
bool GetSignal(int bar_index,
               ...
               double &latest_ATR, double &VWAP_M1)
  {
   ...
   // --- 2) Calculate pullback price (close[1]) ---
   ...
   //   Prepare feature array for XGB model (example size=20)
   double featuresArr[20];
   ArrayInitialize(featuresArr, 0.0);

   // map features — ปรับให้ตรงโมเดลของคุณ
   ...
   // ... เติมจนครบ 20 หรือเท่าที่โมเดลใช้

   // SaveFeatures → Python loop จะ detect และเขียน prediction.json
   ...
   PrintFormat("GetSignal: XGB fallback probs – Buy=%.3f Sell=%.3f", pBuy, pSell);
